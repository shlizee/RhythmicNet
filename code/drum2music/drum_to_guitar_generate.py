...
def extract_events(input_path):
    event2word, word2event = pickle.load(open('./drum2music/dictionary.pkl', 'rb'))
    independent = []
    pos = []
...
def generate(model, temperature, topk, output_path, drums_event):
    ...
    utils.write_midi(
        ...
        output_path=output_path, instrument=24)
...
if __name__ == '__main__':
    ...
    midi_dir = "/data/neurips21/other_long_videos_samples/drums_midi/"
    # "/data/neurips21/other_videos_new_samples/drums_midi/"
    #'/data/neurips21/AIST_new_samples/drums_midi/'
    # list of midi files
    midi_paths = list(find_files_by_extensions(midi_dir, ['.mid', '.midi']))
    # load model
    model_path = 'models/model-guitar-new-emb-noshare-rel.pt'
    ...
    for midi_path in midi_paths:
        ...
        # if asit
        # output_path = 'outputs/drum_to_guitar/aist/{}.mid'.format(output_name)
        # if in the wild
        output_path = "/data/neurips21/other_long_videos_samples/drums_guitar_midi/guitar/{}.mid".format(output_name)
        # "/data/neurips21/other_videos_new_samples/drums_guitar_midi/guitar/{}.mid".format(output_name)
        #'/data/neurips21/AIST_new_samples/drums_guitar_midi/guitar/{}.mid'.format(output_name)
        generate(model, temperature, top_k, output_path, drums_input)
        merge_midi_files(midi_path, output_path,
                         "/data/neurips21/other_long_videos_samples/drums_guitar_midi/merge/{}.mid".format(output_name))
        # merge_midi_files(midi_path, output_path, '/data/neurips21/other_videos_new_samples/drums_guitar_midi/merge/{}.mid'.format(output_name))
        #'/data/neurips21/AIST_new_samples/drums_guitar_midi/merge/{}.mid'.format(output_name))
