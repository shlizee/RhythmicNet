import third_party.remi.chord_recognition as chord_recognition
import numpy as np
import miditoolkit
import copy
...
def read_items(file_path, melody_ins):
    midi_obj = miditoolkit.midi.parser.MidiFile(file_path)
    if not len(midi_obj.time_signature_changes) != 1 and \
        midi_obj.time_signature_changes[0].numerator != 4 and midi_obj.time_signature_changes[0].denominator != 4:
        print("midi files not in 4/4: {0}/{1}".format(midi_obj.time_signature_changes[0].numerator, midi_obj.time_signature_changes[0].denominator))
        return
    # try to extract drums and bass instruments
    ins_names = ['drums', 'piano'] # bass
    instruments = {}
    for ins in midi_obj.instruments:
        # print(ins)
        if ins.program == 0 and ins.is_drum:
            instruments['drums'] = ins
        elif ins.program == melody_ins: # 0
            instruments['piano'] = ins
        # if ins.program == 33:
        #     instruments['bass'] = ins

    if len(list(instruments.keys())) < 2:
        return

    # note
    note_items = {'drums': [], 'piano': []} # bass
    for ins in ins_names:
        notes = instruments[ins].notes
        notes.sort(key=lambda x: (x.start, x.pitch))
        for note in notes:
            note_items[ins].append(Item(
                name='Note',
                start=note.start,
                end=note.end,
                velocity=note.velocity,
                pitch=note.pitch))
        note_items[ins].sort(key=lambda x: x.start)
    # tempo
    tempo_items = []
...
# quantize items
def quantize_items(note_items, ticks=120):
    quantized_note_items = {}
    for ins, items in note_items.items():
        # grid
        ...
        for item in items:
            ...
            item.start += shift
            item.end += shift
        quantized_note_items[ins] = items
    return quantized_note_items
...
def detail_item2event(group, n_downbeat):
    events = []
...
def item2event(groups):
    bass_groups, drums_groups = groups['piano'], groups['drums'] # bass
    events = []
    n_downbeat = 0
    idx = 0
    while idx < min(len(bass_groups), len(drums_groups)):
        seg = {}
        if 'Note' not in [item.name for item in bass_groups[idx][1:-1]] or 'Note' not in [item.name for item in drums_groups[idx][1:-1]]:
            idx += 1
            continue
        else:
            n_downbeat += 1
            seg['piano'] = detail_item2event(bass_groups[idx], n_downbeat) # bass
            seg['drums'] = detail_item2event(drums_groups[idx], n_downbeat)
            events.append(seg)
            idx += 1
...
def write_midi(words, word2event, output_path, prompt_path=None, instrument = 0, is_drum = False):
    ...
    if prompt_path:
        ...
        # write instrument
        inst = miditoolkit.midi.containers.Instrument(instrument, is_drum=is_drum) #0 #33 bass
        inst.notes = notes
